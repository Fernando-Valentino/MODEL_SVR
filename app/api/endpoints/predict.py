...
async def predict_revenue(payload: PredictionInput, token_data: dict = Depends(get_jwt_token)):
    try:
...
async def upload_dataset(
    ...
    token_data: dict = Depends(get_jwt_token)
):
    try:
...
async def train_existing(token_data: dict = Depends(get_jwt_token)):
    try:
        file_path = "research/DATA_PENDAPATAN_PARKIR_PER_HARI_2023-2025.csv"
...
async def load_existing_model(token_data: dict = Depends(get_jwt_token)):
    try:
        from app.core.config import get_settings
        ml_service._load_artifacts()
        settings = get_settings()
...
async def retrain_model(token_data: dict = Depends(get_jwt_token)):
